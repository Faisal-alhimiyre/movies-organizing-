 ...
 *   metadata:v9:season:ratings:tmdb:<id>:<n>:<locale>  24 hours  (TMDB episode + season ratings)
...
(function () {
  ...
  let _anilistRateLimitLogged = false;
  ...
  function markAnilistRateLimited() {
    ...
    window.WatchlistMetadata?.noteAnilistRateLimit?.();
    if (!_anilistRateLimitLogged) {
      _anilistRateLimitLogged = true;
      console.warn("[series-metadata] AniList rate limited — pausing requests for 90s");
      window.setTimeout(() => {
        _anilistRateLimitLogged = false;
      }, ANILIST_COOLDOWN_MS);
    }
  }
  const TMDB_API_BASE = "https://api.themoviedb.org/3";

  /** TMDB genre_ids → English labels (list endpoints only return IDs). */
  const TMDB_GENRE_NAMES = Object.freeze({
    28: "Action",
    12: "Adventure",
    16: "Animation",
    35: "Comedy",
  ...
  function mergeEpisodeLocaleResults(localResult, enResult) {
    ...
    const seasonRating =
      parseExternalRating(localResult.seasonRating) ??
      parseExternalRating(enResult.seasonRating);
    return {
      ...localResult,
  ...
  async function resolveSeriesId(item) {
    ...
    if (ct === "tvSeries" && Number.isFinite(itemTmdbId) && itemTmdbId > 0) {
      return {
        source: "tmdb",
        tmdbId: itemTmdbId,
        imdbId: itemImdbId || null,
        isNegative: false,
      };
    }
    ...
    if (!link && !itemImdbId) return { source: "none", isNegative: true };

    // ── IMDb → TMDb ────────────────────────────────────────────
    const imdbId = itemImdbId || WM?.extractImdbId(link);
    if (imdbId) {
  ...
  function franchiseRootCacheKey(anilistId) {
    return `${FRANCHISE_ROOT_CACHE_PREFIX}${Number(anilistId)}`;
  }

  function readFranchiseRootCacheEntry(anilistId) {
    const id = Number(anilistId);
    if (!Number.isFinite(id)) return null;
    const memKey = String(id);
    if (_anilistFranchiseRootCache.has(memKey)) {
      return _anilistFranchiseRootCache.get(memKey);
    }
    const cached = readCached(franchiseRootCacheKey(id), TTL_RESOLVE);
    if (cached?.rootAnilistId) {
      const entry = {
        rootAnilistId: Number(cached.rootAnilistId),
        rootTitle: cached.rootTitle || "",
      };
      _anilistFranchiseRootCache.set(memKey, entry);
      return entry;
    }
    return null;
  }

  function writeFranchiseRootCacheEntry(anilistId, rootAnilistId, rootTitle = "") {
    const id = Number(anilistId);
    const root = Number(rootAnilistId);
    if (!Number.isFinite(id) || !Number.isFinite(root)) return;
    const entry = { rootAnilistId: root, rootTitle: String(rootTitle || "") };
    _anilistFranchiseRootCache.set(String(id), entry);
    writeSeriesCacheEntry(franchiseRootCacheKey(id), entry, TTL_RESOLVE);
  }

  function readCachedChainIdsFromSeriesMetadata(anilistId) {
    const id = Number(anilistId);
    if (!Number.isFinite(id)) return null;
    for (const locale of ["en", "ar"]) {
      const key = `metadata:v13:series:anilist:${id}:${locale}`;
      const cached = readCached(key, TTL_SERIES);
      const seasons = cached?.payload?.seasons;
      if (!Array.isArray(seasons) || !seasons.length) continue;
      const ids = seasons.map((s) => Number(s.anilistId)).filter(Number.isFinite);
  ...
  function expandFranchiseRootMapFromCachedChains(idToRoot, seedIds = []) {
    const scanned = new Set();
    const queue = [
      ...new Set(
        [...idToRoot.values(), ...seedIds].map(Number).filter(Number.isFinite)
      ),
    ];
    while (queue.length) {
      ...
      const chain = readCachedChainIdsFromSeriesMetadata(candidate);
      if (!chain?.length) continue;
      applyFranchiseChainToRootMap(chain, idToRoot);
      for (const cid of chain) {
        if (!scanned.has(cid)) queue.push(cid);
      }
    }
  }

  function reconcileAnimeFranchiseRootMap(idToRoot, watchlistIds = []) {
    ...
    const seeds = [...new Set(watchlistIds.map(Number).filter(Number.isFinite))];
    for (const id of seeds) {
      find(id);
      const chain = readCachedChainIdsFromSeriesMetadata(id);
      if (chain?.length) {
        const chainRoot = Number(chain[0]);
        for (const cid of chain) union(chainRoot, cid);
        union(id, chainRoot);
      }
    }
    for (const [id, root] of idToRoot.entries()) {
      union(id, root);
    }
    ...
    for (const id of seeds) {
      const root = find(id);
      if (root != null) reconciled.set(id, root);
    }
    return reconciled;
  }

  function applyMultiEntryChainsToRootMap(idToRoot, watchlistIds = []) {
    for (const id of watchlistIds) {
      const chain = readCachedChainIdsFromSeriesMetadata(id);
      if (!chain || chain.length < 2) continue;
      const canonical = Number(chain[0]);
      if (!Number.isFinite(canonical)) continue;
      for (const cid of chain) {
        const current = idToRoot.get(cid);
        if (!idToRoot.has(cid) || current === cid) {
          idToRoot.set(cid, canonical);
        }
      }
    }
  }
  ...
  /** OMDb often indexes anime under simplified romaji (Busou → Buso). */
  function omdbTitleVariants(title) {
    const raw = String(title || "").trim();
    if (raw.length < 2) return [];
    const out = [raw];
    const simplified = raw.replace(/ou/gi, "o").replace(/uu/gi, "u");
    if (simplified !== raw && simplified.length >= 2) out.push(simplified);
    return out;
  }

  async function omdbPayloadForImdbId(imdbId, options = {}) {
    const id = String(imdbId || "").toLowerCase();
    if (!/^tt\d{7,10}$/.test(id)) return null;
    const WM = window.WatchlistMetadata;
    if (!WM?.getMetadata) return null;
    try {
      return (await WM.getMetadata(id, options)) || null;
    } catch {
      return null;
    }
  }
  ...
  async function searchOmdbImdbByTitle(title, year = null, options = {}) {
    ...
    if (options.omdbOnly) return null;
    ...
    // Only accept suggestions OMDb can resolve (filters bad AKA matches).
    for (const q of variants) {
      const found = await searchImdbSuggestImdbId(q, year, types);
      if (found) return found;
    }
    return null;
  }
  ...
  async function searchImdbSuggestImdbId(q, year, types) {
    ...
    const cacheKey = `metadata:v2:resolve:imdb:suggest:${q.toLowerCase()}:${year || ""}:${typesKey}`;
    const cached = readCached(cacheKey, TTL_RESOLVE);
    ...
    for (const row of res.results || []) {
      ...
      const candidate = String(row.imdbId || "").toLowerCase();
      if (!/^tt\d{7,10}$/.test(candidate)) continue;
      const meta = await omdbPayloadForImdbId(candidate);
      if (!meta) continue;
      // Prefer an id that already has an IMDb score in OMDb.
      if (meta.rating) {
        writeSeriesCacheEntry(cacheKey, { imdbId: candidate }, TTL_RESOLVE);
        return candidate;
      }
      if (!fallbackId) fallbackId = candidate;
    }

    if (fallbackId) {
      writeSeriesCacheEntry(cacheKey, { imdbId: fallbackId }, TTL_RESOLVE);
      return fallbackId;
    }
  ...
  async function searchOmdbImdbByTitleTyped(q, year, omdbType) {
    const apiKey = getOmdbKey();
    if (!apiKey || q.length < 2) return null;

    const cacheKey = `metadata:v9:resolve:imdb:omdbsearch:${q.toLowerCase()}:${year || ""}:${omdbType}`;
    const cached = readCached(cacheKey, TTL_RESOLVE);
    if (cached?.imdbId) return cached.imdbId;
    ...
    try {
      ...
      for (const hit of json.Search || []) {
        ...
        if (String(hit.Type || "").toLowerCase() === omdbType) score += 5;
  ...
  async function searchOmdbImdbCandidates(itemOrHints, year = null, options = {}) {
    const title = String(itemOrHints?.title || itemOrHints || "").trim();
    const altTitle = String(itemOrHints?.altTitle || "").trim();
    const yearHint = itemOrHints?.year ?? year ?? null;
    const titles = [];
    const seen = new Set();
    for (const base of [title, altTitle]) {
      for (const variant of omdbTitleVariants(base)) {
    ...
    for (const candidate of titles) {
      const withYear = await searchOmdbImdbByTitle(candidate, yearHint, options);
      if (withYear) return withYear;
    }
    if (yearHint != null && yearHint !== "") {
      for (const candidate of titles) {
        const loose = await searchOmdbImdbByTitle(candidate, null, options);
  ...
  function anilistMediaTitle(media) {
    return (
      media?.title?.english ||
      media?.title?.romaji ||
      media?.title?.native ||
  ...
  function franchiseRootMediaFromCache(anilistId) {
    const cached = readFranchiseRootCacheEntry(anilistId);
    if (!cached?.rootAnilistId) return null;
    return {
      id: cached.rootAnilistId,
      title: {
        english: cached.rootTitle,
        romaji: cached.rootTitle,
        native: cached.rootTitle,
      },
    };
  }

  async function walkAnilistTvPrequelRoot(anilistId, maxHops = 12, options = {}) {
    let currentId = Number(anilistId);
    if (!Number.isFinite(currentId)) {
      return { media: null, completed: false, rateLimited: false, visitedIds: [] };
    }

    const skipCache = Boolean(options.skipCache);
    const hopCached = !skipCache && readFranchiseRootCacheEntry(currentId);
    if (hopCached?.rootAnilistId) {
      return {
        media: franchiseRootMediaFromCache(currentId),
        completed: true,
        rateLimited: false,
    ...
    for (let hop = 0; hop < maxHops; hop++) {
      ...
      const cachedHop = !skipCache && readFranchiseRootCacheEntry(currentId);
      if (cachedHop?.rootAnilistId) {
        rootMedia = franchiseRootMediaFromCache(currentId);
        completed = true;
        break;
      }
      ...
      if (!media) {
        return {
          media: rootMedia,
          completed: false,
          rateLimited: isAnilistRateLimited(),
          visitedIds,
        };
      }
      rootMedia = media;
    ...
    if (rootMedia && completed) {
      const rootId = Number(rootMedia.id);
      const rootTitle = anilistMediaTitle(rootMedia);
  ...
  async function resolveAnimeFranchiseRoot(anilistId, options = {}) {
    const id = Number(anilistId);
    ...
    if (!options.skipCache) {
      const cached = readFranchiseRootCacheEntry(id);
      if (cached?.rootAnilistId) {
        return {
          rootAnilistId: cached.rootAnilistId,
          rootTitle: cached.rootTitle || "",
          rateLimited: false,
          incomplete: false,
          fromCache: true,
    ...
    if (!walk.completed) {
      return {
        rootAnilistId: null,
        rootTitle: "",
        rateLimited: Boolean(walk.rateLimited),
        incomplete: true,
      };
    }
  ...
  const FRANCHISE_ROOT_API_WALK_LIMIT = 10;

  function collectFranchiseRewalkCandidates(ids, idToRoot) {
    const idSet = new Set(ids);
    const candidates = new Set();
    for (const id of ids) {
      const chain = readCachedChainIdsFromSeriesMetadata(id);
      if (!chain || chain.length < 2) continue;
      const chainOnList = chain.filter((cid) => idSet.has(cid));
      if (chainOnList.length < 2) continue;
      for (const cid of chainOnList) candidates.add(cid);
    }
    return ids.filter(
      (id) => !idToRoot.has(id) || (idToRoot.get(id) === id && candidates.has(id))
    );
  }

  async function buildAnimeFranchiseRootMap(anilistIds, options = {}) {
    const ids = [...new Set(anilistIds.map(Number).filter(Number.isFinite))];
    const idToRoot = new Map();
    let rateLimited = false;

    expandFranchiseRootMapFromCachedChains(idToRoot, ids);
    applyMultiEntryChainsToRootMap(idToRoot, ids);

    for (const id of ids) {
      if (idToRoot.has(id)) continue;
      const cached = readFranchiseRootCacheEntry(id);
      if (cached?.rootAnilistId) {
        idToRoot.set(id, cached.rootAnilistId);
      }
    }
    ...
    const selfRootIds = ids.filter((id) => idToRoot.get(id) === id);
    let unresolved = collectFranchiseRewalkCandidates(ids, idToRoot);
    unresolved.sort((a, b) => a - b);

    let apiWalks = 0;
    for (const id of unresolved) {
      if (apiWalks >= FRANCHISE_ROOT_API_WALK_LIMIT) break;
      if (isAnilistRateLimited()) {
        rateLimited = true;
        break;
      }
      const franchise = await resolveAnimeFranchiseRoot(id, {
        skipCache: selfRootIds.includes(id),
      });
      apiWalks += 1;
      if (franchise.rateLimited) {
        rateLimited = true;
        break;
      }
      if (!franchise.rootAnilistId) continue;
      idToRoot.set(id, franchise.rootAnilistId);
      writeFranchiseRootCacheEntry(id, franchise.rootAnilistId, franchise.rootTitle || "");
      expandFranchiseRootMapFromCachedChains(idToRoot, [franchise.rootAnilistId, id]);
      applyMultiEntryChainsToRootMap(idToRoot, ids);
      if (options.throttle !== false) {
        await delayMs(ANIME_FRANCHISE_RESOLVE_DELAY_MS);
      }
    }

    let reconciled = reconcileAnimeFranchiseRootMap(idToRoot, ids);
    applyMultiEntryChainsToRootMap(reconciled, ids);
    reconciled = reconcileAnimeFranchiseRootMap(reconciled, ids);

    for (const [id, root] of reconciled.entries()) {
      const prev = readFranchiseRootCacheEntry(id);
      if (!prev || prev.rootAnilistId !== root) {
        writeFranchiseRootCacheEntry(
          id,
          root,
          readFranchiseRootCacheEntry(root)?.rootTitle || prev?.rootTitle || ""
        );
      }
    }
    ...
    return {
      idToRoot: reconciled,
      rateLimited,
      unresolvedIds: ids.filter((id) => !reconciled.has(id)),
    };
  }

  function buildBatchFranchiseRootMapFromCache(anilistIds) {
    const ids = [...new Set(anilistIds.map(Number).filter(Number.isFinite))];
    const idToRoot = new Map();
    expandFranchiseRootMapFromCachedChains(idToRoot, ids);
    applyMultiEntryChainsToRootMap(idToRoot, ids);
    for (const id of ids) {
      if (idToRoot.has(id)) continue;
      const cached = readFranchiseRootCacheEntry(id);
      if (cached?.rootAnilistId) idToRoot.set(id, cached.rootAnilistId);
    }
    expandFranchiseRootMapFromCachedChains(idToRoot, ids);
    applyMultiEntryChainsToRootMap(idToRoot, ids);
    return reconcileAnimeFranchiseRootMap(idToRoot, ids);
  }
  ...
  async function resolveLinkedImdbId(resolution, item = null, hints = {}) {
    ...
    const skipAnilist = hints.skipAnilist === true;
    const anilistId = resolution?.anilistId;
    const titleHint = String(hints.title || item?.title || "").trim();
    const yearHint = hints.year ?? item?.year ?? null;
    const searchHints = {
      title: titleHint,
      altTitle: hints.altTitle || item?.altTitle || "",
      year: yearHint,
    };
    ...
    if (seasonScoped && anilistId && !skipAnilist) {
      const fromCour = await resolveImdbFromAnilistMedia(Number(anilistId), {
        ...hints,
        title: titleHint,
        year: yearHint,
      }, {
    ...
    // When we have an AniList id, skip English-title suggest here and let
    // resolveImdbFromAnilistMedia try romaji/OMDb-validated candidates.
    if (!anilistId || skipAnilist) {
      const fromOmdbEarly = await searchOmdbImdbCandidates(searchHints, yearHint, {
        omdbOnly: Boolean(anilistId),
      });
      if (fromOmdbEarly) {
        if (anilistId) {
          const cacheKey = `metadata:v12:resolve:imdb:anilist:${anilistId}`;
          writeSeriesCacheEntry(cacheKey, { imdbId: fromOmdbEarly }, TTL_RESOLVE);
        }
        return fromOmdbEarly;
      }
    }

    if (!anilistId || skipAnilist) return null;

    return resolveImdbFromAnilistMedia(Number(anilistId), {
      ...hints,
      title: titleHint,
      year: yearHint,
    }, {
      ...
      skipAnilist,
      requireRating: hints.requireRating === true,
      forceRefresh: hints.forceRefresh === true,
    });
  }
  ...
  async function resolveImdbFromAnilistMedia(anilistId, hints = {}, opts = {}) {
    ...
    const skipAnilist = opts.skipAnilist === true || hints.skipAnilist === true;
    const WM = window.WatchlistMetadata;
    // v12: do not trust AniList/suggest IMDb ids that OMDb cannot resolve
    // (e.g. Arms Alchemy → wrong tt11916660 instead of Buso Renkin tt0877507).
    const cacheKey = `metadata:v12:resolve:imdb:anilist:${anilistId}`;
    const cached = readCached(cacheKey, TTL_RESOLVE);
    if (cached?.imdbId && !opts.forceRefresh) {
      if (!opts.requireRating) return cached.imdbId;
      const cachedMeta = await omdbPayloadForImdbId(cached.imdbId);
      if (cachedMeta?.rating) return cached.imdbId;
    }

    const titleHint = String(hints.title || "").trim();
    const yearHint = hints.year ?? null;
    // Literal OMDb only here — IMDb suggest on English anime titles often
    // returns an unrelated tt id that still resolves in OMDb.
    const fromOmdbHint = await searchOmdbImdbCandidates(
      { title: titleHint, altTitle: hints.altTitle || "", year: yearHint },
      yearHint,
      { omdbOnly: true }
    );
    if (fromOmdbHint && skipAnilist) {
      writeSeriesCacheEntry(cacheKey, { imdbId: fromOmdbHint }, TTL_RESOLVE);
      return fromOmdbHint;
    }

    if (skipAnilist) return fromOmdbHint || null;
    if (isAnilistRateLimited()) return fromOmdbHint || null;
    if (window.WatchlistMetadata?.getAnilistQueueStatus?.()?.paused) {
      return fromOmdbHint || null;
    }
    ...
    try {
      ...
      const media = data?.Media;
      if (!media) return fromOmdbHint || null;
      ...
      // Prefer OMDb title search across English/romaji/native before trusting
      // AniList's IMDb external link (often missing or wrong for anime).
      const mediaTitles = [
        hints.title,
        hints.altTitle,
        media.title?.english,
      ...
      for (const t of mediaTitles) {
        for (const v of omdbTitleVariants(t)) {
      ...
      let bestRated = null;
      let bestAny = null;
      // Pass 1: OMDb literal search only (romaji/Buso variants) — avoids English
      // AKA suggest hits that point at the wrong title.
      ...
      // Pass 2: allow IMDb suggest only if OMDb literal search missed.
      if (!bestRated) {
        for (const passTitle of titlePasses) {
      ...
      const fromMediaTitles = bestRated || (!opts.requireRating ? bestAny : null);
      if (fromMediaTitles) {
        writeSeriesCacheEntry(cacheKey, { imdbId: fromMediaTitles }, TTL_RESOLVE);
        return fromMediaTitles;
      }

      for (const link of media.externalLinks || []) {
        const fromUrl = WM?.extractImdbId?.(link?.url);
        if (!fromUrl) continue;
        const meta = await omdbPayloadForImdbId(fromUrl);
        if (!meta) continue;
        if (opts.requireRating && !meta.rating) continue;
        writeSeriesCacheEntry(cacheKey, { imdbId: fromUrl }, TTL_RESOLVE);
        return fromUrl;
      }

      if (!allowFranchiseFallback) return fromOmdbHint || null;

      if (isAnilistRateLimited()) return fromOmdbHint || null;

      const walk = await walkAnilistTvPrequelRoot(Number(anilistId));
      const rootMedia = walk?.completed ? walk.media : null;
      if (rootMedia && Number(rootMedia.id) !== Number(anilistId)) {
        ...
        if (franchiseImdb) {
          const meta = await omdbPayloadForImdbId(franchiseImdb);
          if (meta && (!opts.requireRating || meta.rating)) {
  ...
  async function enrichEpisodesWithTmdbRatings(
    ...
    const needsSeasonRating = parseExternalRating(result.seasonRating) == null;
    if (!needsRating && !needsSeasonRating) return result;
    ...
    const ratingPayload = await fetchEpisodeRatingMap(tmdbId, seasonNumber, locale);
    if (!ratingPayload) return result;

    const byEpisode = ratingPayload.byEpisode;
    const seasonRating =
      parseExternalRating(result.seasonRating) ??
      parseExternalRating(ratingPayload.seasonRating);

    let next = { ...result };
    if (seasonRating != null) {
      next.seasonRating = seasonRating;
      next.seasonRatingSource = result.seasonRatingSource || "tmdb";
    }

    if (byEpisode?.size) {
      next.episodes = (result.episodes || []).map((ep) => {
  ...
  async function enrichEpisodeRatings(
    ...
    // TMDB for ratings. Skip OMDb when TMDB already covered ratings + air/titles.
    ...
    const eps = enriched?.episodes || [];
    const needsOmdbFill = eps.some((ep) => {
      if (!ep.airDate) return true;
      if (
        locale !== "ar" &&
        (!ep.title || isGenericEpisodeTitle(ep.title, Number(ep.episodeNumber)))
      ) {
        return true;
      }
      return false;
  ...
  async function fetchEpisodeRatingMap(tmdbId, seasonNumber, locale) {
    // v9: also caches season-level vote_average from the same TMDB season call.
    const cacheKey = `metadata:v9:season:ratings:tmdb:${tmdbId}:${seasonNumber}:${locale}`;
    const cached = readCached(cacheKey, TTL_EPISODES);
    const fromCache = ratingMapFromCacheObject(cached?.byEpisode);
    if (fromCache || parseExternalRating(cached?.seasonRating) != null) {
      return {
        byEpisode: fromCache || new Map(),
        seasonRating: parseExternalRating(cached?.seasonRating),
      };
    }
    ...
    if (getTmdbKey()) {
      ...
      seasonRating = parseExternalRating(json?.vote_average);
      if ((!byEpisode?.size || seasonRating == null) && locale !== "en") {
        const enJson = await fetchTmdb(`tv/${tmdbId}/season/${seasonNumber}`, {
          language: "en-US",
        });
        if (!byEpisode?.size && enJson?.episodes?.length) {
          byEpisode = buildTmdbRatingMap(enJson.episodes);
        }
        if (seasonRating == null) {
          seasonRating = parseExternalRating(enJson?.vote_average);
        }
      }
    } else if (window.WatchlistTmdb?.fetchSeasonRatings) {
      let payload = await window.WatchlistTmdb.fetchSeasonRatings(
        tmdbId,
        seasonNumber,
        locale
      );
      if ((!payload?.episodes?.length || payload.seasonRating == null) && locale !== "en") {
        const enPayload = await window.WatchlistTmdb.fetchSeasonRatings(
          tmdbId,
          seasonNumber,
          "en"
        );
        if (enPayload) {
          payload = {
            episodes: payload?.episodes?.length ? payload.episodes : enPayload.episodes,
            seasonRating: payload?.seasonRating ?? enPayload.seasonRating,
          };
        }
      }
      if (payload?.episodes?.length) {
        byEpisode = buildTmdbRatingMap(payload.episodes);
      }
      seasonRating = parseExternalRating(payload?.seasonRating);
    }

    if (!byEpisode?.size && seasonRating == null) return null;

    writeSeriesCacheEntry(
      cacheKey,
      {
        ...(byEpisode?.size ? { byEpisode: Object.fromEntries(byEpisode) } : {}),
        ...(seasonRating != null ? { seasonRating } : {}),
      },
      TTL_EPISODES
    );
    return {
      byEpisode: byEpisode || new Map(),
      seasonRating,
    };
  }
  ...
  async function enrichEpisodesWithOmdbRatings(
    ...
    return {
      ...
      episodes: (result.episodes || []).map((ep) => {
        ...
        // Do not use OMDb for episode scores — season bulk ratings are sparse/misleading.
        // Keep OMDb only for air dates / English titles when needed.
  ...
  function normalizeTmdbSeasonEpisodes(json, tmdbId, season, fallbackPoster = "") {
    ...
    const seasonRating = parseExternalRating(json.vote_average);
    ...
    return {
      ...
      ...(seasonRating != null
        ? { seasonRating, seasonRatingSource: "tmdb" }
        : {}),
    };
  }
  ...
  function animeFranchiseBaseTitle(nodeOrTitle) {
    let raw = "";
    if (typeof nodeOrTitle === "string") {
      raw = nodeOrTitle;
    } else if (nodeOrTitle) {
      raw =
        nodeOrTitle.title?.english ||
        nodeOrTitle.title?.romaji ||
        nodeOrTitle.title?.native ||
        "";
    }
    const trimmed = String(raw || "").trim();
    const colonIdx = trimmed.indexOf(":");
    const primary = colonIdx >= 0 ? trimmed.slice(0, colonIdx) : trimmed;
  ...
  function normalizeAnilistRelatedMovie(node) {
    ...
    const genres = Array.isArray(node.genres)
      ? node.genres.map((g) => String(g || "").trim()).filter(Boolean).slice(0, 4)
      : [];
    const format = anilistSeasonFormat(node?.format);
    const episodeCount = parsePositiveCount(node?.episodes);
    const isMovie = format === "MOVIE" || format === "SPECIAL";
    return {
      source: "anilist",
      ...
      genres,
      ageRating: node?.isAdult ? "R" : "",
      episodeCount: episodeCount || null,
      seasonCount: isMovie ? null : 1,
  ...
  function similarTitleYearSortKey(entry) {
    const y = parseInt(String(entry?.year || "").slice(0, 4), 10);
    return Number.isFinite(y) ? y : 9999;
  }

  /**
   * AniList recommendations often include multiple seasons of the same show as
   * separate media. Collapse to one card per franchise root (prefer root /
   * earliest season) so Similar never lists X S1…S9 as nine entries.
   */
  async function dedupeAnilistSimilarByFranchise(titles) {
    if (!Array.isArray(titles) || titles.length < 2) return titles || [];

    const ids = titles
      .map((t) => Number(t.anilistId))
      .filter((id) => Number.isFinite(id));
    if (ids.length < 2) return titles;

    const mapResult = await buildAnimeFranchiseRootMap(ids, { throttle: true });
    const idToRoot = mapResult?.idToRoot || new Map();
    ...
    for (const entry of titles) {
      const id = Number(entry.anilistId);
      if (!Number.isFinite(id)) {
        order.push({ kind: "singleton", entry });
        continue;
      }
      const root = idToRoot.get(id) ?? id;
    ...
    for (const item of order) {
      ...
      if (group.members.length === 1) {
        ...
        const rootId = Number(group.root);
        if (
          Number.isFinite(rootId) &&
          Number(only.anilistId) !== rootId
        ) {
          ...
          out.push({
            ...
            ...(rootEntry
              ? {
                  ...
                  poster: rootEntry.poster,
                  year: rootEntry.year,
                  overview: rootEntry.overview,
                  runtimeMinutes: rootEntry.runtimeMinutes,
                  score: rootEntry.score,
                  format: rootEntry.format,
                }
              : {}),
            anilistId: rootId,
            pick: { anilistId: rootId, source: "anilist" },
          });
        } else {
          out.push(only);
        }
      ...
      const rootId = Number(group.root);
      let best =
        group.members.find((m) => Number(m.anilistId) === rootId) || null;
      if (!best) {
        best = [...group.members].sort(
          (a, b) => similarTitleYearSortKey(a) - similarTitleYearSortKey(b)
        )[0];
      }
      const rootEntry = byId.get(rootId);
      if (Number.isFinite(rootId) && Number(best.anilistId) !== rootId) {
        out.push({
          ...best,
          ...(rootEntry
            ? {
  ...
  async function fetchAnilistSimilarTitles(anilistId, locale, excludeId = null) {
    const cacheKey = `metadata:v2:similar:anilist:${anilistId}:${locale}`;
    const cached = readCached(cacheKey, TTL_SERIES);
    if (cached?.titles) {
      return {
        state: cached.titles.length ? ResultState.AVAILABLE : ResultState.NO_SEASONS,
    ...
    const seen = new Set();
    const exclude = Number(excludeId) || Number(anilistId);
    const rawTitles = [];
    for (const row of nodes) {
      const node = row?.mediaRecommendation;
      if (!node?.id || node.type !== "ANIME") continue;
      const id = Number(node.id);
      if (!Number.isFinite(id) || id === exclude || seen.has(id)) continue;
      seen.add(id);
      rawTitles.push(normalizeAnilistSimilarTitle(node));
    }

    const titles = await dedupeAnilistSimilarByFranchise(rawTitles);

    writeSeriesCacheEntry(cacheKey, { titles }, TTL_SERIES);
    return {
      state: titles.length ? ResultState.AVAILABLE : ResultState.NO_SEASONS,
      titles,
    };
  }

  function resolveItemImdbId(item, resolution) {
    const fromRes = String(resolution?.imdbId || "").trim().toLowerCase();
    if (/^tt\d{6,10}$/.test(fromRes)) return fromRes;

    const raw = String(item?.imdbId || "").trim().toLowerCase();
    if (/^tt\d{6,10}$/.test(raw)) return raw;
    if (/^\d{6,10}$/.test(raw)) return `tt${raw}`;
  ...
  function resolveItemTmdbId(item, resolution, preferMediaType) {
    const fromRes = Number(resolution?.tmdbId);
    if (Number.isFinite(fromRes) && fromRes > 0) {
      return { tmdbId: fromRes, mediaType: preferMediaType };
    }
    const fromItem = Number(item?.tmdbId);
    if (Number.isFinite(fromItem) && fromItem > 0) {
      return { tmdbId: fromItem, mediaType: preferMediaType };
    }
    const parsed = window.WatchlistMetadata?.extractTmdbId?.(item?.link);
    if (parsed?.tmdbId) {
      return {
        tmdbId: Number(parsed.tmdbId),
        mediaType: parsed.mediaType === "movie" ? "movie" : parsed.mediaType === "tv" ? "tv" : preferMediaType,
      };
    }
    return { tmdbId: null, mediaType: preferMediaType };
  }

  function mapTmdbSimilarRows(rows, mediaType, selfTmdbId) {
    const IMG = "https://image.tmdb.org/t/p/w342";
    const seen = new Set();
    const titles = [];
    for (const row of rows) {
      if (!row || typeof row !== "object") continue;
      const id = Number(row.id || row.tmdbId) || 0;
      if (!id || seen.has(id) || id === selfTmdbId) continue;
      seen.add(id);
      const title = String(row.title || row.name || "").trim();
      if (!title) continue;
      const date = String(row.release_date || row.first_air_date || "");
      const year = date.length >= 4 ? date.substring(0, 4) : String(row.year || "");
      const posterPath = String(row.poster_path || "");
      const poster = row.poster
        ? String(row.poster)
        : posterPath
          ? `${IMG}${posterPath}`
          : "";
      const vote = Number(row.vote_average);
      const score =
        row.score != null
          ? Number(row.score)
          : Number.isFinite(vote) && vote > 0
            ? Math.round(vote * 10)
            : null;
      const genres = Array.isArray(row.genres) && row.genres.length
        ? row.genres.map((g) => String(g || "").trim()).filter(Boolean)
        : genresFromTmdbIds(row.genre_ids || row.genreIds);
      const seasonCount =
        parsePositiveCount(row.seasonCount ?? row.number_of_seasons) || null;
      const episodeCount =
        parsePositiveCount(row.episodeCount ?? row.number_of_episodes) || null;
      const ageRating = String(row.ageRating || row.contentRating || "").trim();
      const runtimeMinutes =
        parsePositiveCount(row.runtimeMinutes ?? row.runtime) || null;
      titles.push({
        source: "tmdb",
        tmdbId: id,
        title,
  ...
  function pickTmdbContentRating(details) {
    const results = details?.content_ratings?.results;
    if (!Array.isArray(results)) return "";
    const prefer = ["US", "GB", "CA", "AU"];
    for (const iso of prefer) {
      const hit = results.find(
        (r) => String(r?.iso_3166_1 || "").toUpperCase() === iso
      );
      const rating = String(hit?.rating || "").trim();
  ...
  async function enrichTmdbTvSimilarMeta(titles, locale) {
    if (!Array.isArray(titles) || !titles.length || !getTmdbKey()) return titles;
    const lang = String(locale || "en").startsWith("ar") ? "ar-SA" : "en-US";
    const slice = titles.slice(0, 12);
    await Promise.all(
      slice.map(async (entry) => {
        const id = Number(entry?.tmdbId);
        if (!Number.isFinite(id) || id <= 0) return;
        if (entry.seasonCount && entry.episodeCount && entry.ageRating) return;
        const details = await fetchTmdb(`tv/${id}`, {
          language: lang,
          append_to_response: "content_ratings",
        });
        if (!details) return;
        const seasons = parsePositiveCount(details.number_of_seasons);
        const episodes = parsePositiveCount(details.number_of_episodes);
        const age = pickTmdbContentRating(details);
        if (seasons) entry.seasonCount = seasons;
        if (episodes) entry.episodeCount = episodes;
        if (age) entry.ageRating = age;
  ...
  async function fetchTmdbSimilarViaDirectApi(tmdbId, mediaType, locale) {
    if (!getTmdbKey() || !tmdbId) return null;
    const lang = String(locale || "en").startsWith("ar") ? "ar-SA" : "en-US";
    const [similarJson, recJson] = await Promise.all([
      fetchTmdb(`${mediaType}/${tmdbId}/similar`, { language: lang, page: "1" }),
      fetchTmdb(`${mediaType}/${tmdbId}/recommendations`, {
        language: lang,
        page: "1",
      }),
    ]);
    const rows = [
      ...
      ...(Array.isArray(similarJson?.results) ? similarJson.results : []),
    ];
    let titles = mapTmdbSimilarRows(rows, mediaType, tmdbId);
    if (mediaType === "tv" && titles?.length) {
      titles = await enrichTmdbTvSimilarMeta(titles, locale);
    }
    return titles;
  }

  async function resolveTmdbIdForSimilar(imdbId, mediaType) {
    if (!imdbId) return null;
    if (getTmdbKey()) {
      const findJson = await fetchTmdb(`find/${imdbId}`, {
        external_source: "imdb_id",
      });
      const movieHit = Array.isArray(findJson?.movie_results)
        ? findJson.movie_results[0]
        : null;
      const tvHit = Array.isArray(findJson?.tv_results)
        ? findJson.tv_results[0]
        : null;
      if (mediaType === "movie" && movieHit?.id) return Number(movieHit.id);
      if (mediaType === "tv" && tvHit?.id) return Number(tvHit.id);
      if (movieHit?.id) return Number(movieHit.id);
      if (tvHit?.id) return Number(tvHit.id);
      return null;
    }
    const Tmdb = window.WatchlistTmdb;
    if (Tmdb?.resolveByImdb && mediaType === "tv") {
      return Tmdb.resolveByImdb(imdbId);
    }
    return null;
  }

  async function fetchTmdbSimilarTitles(resolution, item, locale) {
    const preferMediaType = item?.contentType === "movies" ? "movie" : "tv";
    const imdbId = resolveItemImdbId(item, resolution);
    let { tmdbId, mediaType } = resolveItemTmdbId(item, resolution, preferMediaType);

    const cacheKey = `metadata:v4:similar:tmdb:${mediaType}:${tmdbId || imdbId || "x"}:${locale}`;
    const cached = readCached(cacheKey, TTL_SERIES);
    if (Array.isArray(cached?.titles) && cached.titles.length) {
      return {
        state: ResultState.AVAILABLE,
        titles: cached.titles,
        isStale: isStale(cacheKey, TTL_SERIES),
    ...
    if ((!tmdbId || tmdbId <= 0) && imdbId) {
      tmdbId = await resolveTmdbIdForSimilar(imdbId, mediaType);
    }

    // Prefer direct TMDB when a client key is configured (no edge deploy needed).
    if (tmdbId && getTmdbKey()) {
      const titles = await fetchTmdbSimilarViaDirectApi(tmdbId, mediaType, locale);
      if (titles?.length) {
        writeSeriesCacheEntry(cacheKey, { titles }, TTL_SERIES);
        return { state: ResultState.AVAILABLE, titles };
      }
    }

    const Tmdb = window.WatchlistTmdb;
    if (Tmdb?.fetchSimilar && Tmdb.isAvailable?.()) {
      const result = await Tmdb.fetchSimilar({
      ...
      if (result?.ok) {
        const titles = mapTmdbSimilarRows(
          result.results || [],
          mediaType,
          tmdbId
        );
        ...
        return { state: ResultState.NO_SEASONS, titles: [] };
      }
    }
  ...
  /**
   * "More like this" recommendations — AniList for anime, TMDB (IMDb-linked) for
   * movies/TV. Same card shape as related movies for the Similar tab.
   */
  async function fetchSimilarTitles(resolution, item, locale = "en") {
    if (!resolution || resolution.isNegative) {
      // For movies, allow fallback — fetchTmdbSimilarTitles extracts IDs from the item directly.
      if (item?.contentType === "movies") {
        return fetchTmdbSimilarTitles(resolution || {}, item, locale);
      }
      return { state: ResultState.INVALID_ID, titles: [] };
    }
    ...
    if (anilistId) {
      return fetchAnilistSimilarTitles(anilistId, locale, anilistId);
    }
  ...
  function parseExternalRating(raw) {
    const n = Number(raw);
    if (!Number.isFinite(n) || n <= 0 || n > 10) return null;
    return Math.round(n * 10) / 10;
  }
  ...
  async function anilistQuery(query, variables) {
    ...
    const metaGate = window.WatchlistMetadata?.getAnilistQueueStatus?.();
    if (metaGate?.paused) return null;
  ...
  function episodesResultPayload(result) {
    ...
    return {
      ...
      ...(parseExternalRating(result.seasonRating) != null
        ? {
            seasonRating: parseExternalRating(result.seasonRating),
            seasonRatingSource: result.seasonRatingSource || "tmdb",
          }
        : {}),
    };
  }
  ...
  function parseCachedEpisodesResult(cached, { isStale = false, forceState = null } = {}) {
    ...
    return {
      ...
      ...(parseExternalRating(payload.seasonRating) != null
        ? {
            seasonRating: parseExternalRating(payload.seasonRating),
            seasonRatingSource: payload.seasonRatingSource || "tmdb",
          }
        : {}),
      isStale,
    };
    ...
    if (ct === "tvSeries" || ct === "anime") {
      ...
      if (ct === "anime") {
        ...
        const anilistId =
          ...
          null;
        if (anilistId) {
          let linkedImdb = imdbId;
          let imdbMeta = linkedImdb ? await omdbPayloadForImdbId(linkedImdb) : null;
          const needsRating = !item?.imdbRating;
          if (needsRating || !imdbMeta) {
            if (!imdbMeta?.rating) {
              const resolved = await resolveLinkedImdbId(
                { anilistId: Number(anilistId) },
                item,
                {
                  title: item?.title,
                  ...
                  year: item?.year,
                  skipAnilist: isAnilistRateLimited(),
                  requireRating: needsRating,
                  forceRefresh: true,
                }
              );
              if (resolved && resolved !== linkedImdb) {
                linkedImdb = resolved;
                imdbMeta = await omdbPayloadForImdbId(linkedImdb, { refresh: true });
              } else if (!linkedImdb && resolved) {
                linkedImdb = resolved;
                imdbMeta = await omdbPayloadForImdbId(linkedImdb);
              } else if (linkedImdb && !imdbMeta && resolved) {
                linkedImdb = resolved;
                imdbMeta = await omdbPayloadForImdbId(linkedImdb, { refresh: true });
              }
            }
  ...
  function isUsableAnimePosterUrl(url) {
  ...
  function isRegularAnimeSeason(season) {
    return Boolean(
      season &&
  ...
  /**
   * Pick the best main-card poster when the stored poster is missing or broken.
   * Prefers root S1, selected season, then first regular season (not specials/movies).
   */
  function pickAnimeMainPosterFallback(item, seasons, options = {}) {
    const WM = window.WatchlistMetadata;
    const normalize = (url) => WM?.upgradePosterForStorage?.(url, item) || url;
    const regular = (seasons || []).filter(isRegularAnimeSeason);
    const priority = [];

    const seasonOne = regular.find((s) => s.seasonNumber === 1);
    if (seasonOne?.poster) {
      priority.push({ url: seasonOne.poster, source: "season_1" });
    }

    const selNum =
      options.selectedSeason ?? item?.lastSelectedSeason ?? item?.selectedSeason;
    if (selNum != null) {
      const selected = regular.find((s) => s.seasonNumber === Number(selNum));
      if (selected?.poster) {
        priority.push({ url: selected.poster, source: `selected_season_${selNum}` });
      }
    }

    for (const season of regular) {
      if (season.poster) {
        priority.push({ url: season.poster, source: `season_${season.seasonNumber}` });
      }
    }

    if (options.seriesPoster) {
      priority.push({ url: options.seriesPoster, source: "series_meta" });
    }

    const healedPoster = normalize(item?.poster || "");
    if (isUsableAnimePosterUrl(healedPoster)) {
      priority.push({ url: healedPoster, source: "item_poster" });
    }

    const healedCard = normalize(item?.cardPoster || "");
    if (isUsableAnimePosterUrl(healedCard)) {
      priority.push({ url: healedCard, source: "card_poster" });
    }

    const seen = new Set();
    for (const cand of priority) {
      const url = normalize(cand.url);
      if (!isUsableAnimePosterUrl(url) || seen.has(url)) continue;
      seen.add(url);
      return { poster: url, source: cand.source };
    }
    return null;
  }

  async function collectAnimeFranchiseTvIds(rootAnilistId, maxNodes = 40) {
    const root = Number(rootAnilistId);
    if (!Number.isFinite(root)) return [];
    const memKey = `franchise-tv:${root}`;
    if (_anilistFranchiseTvIds.has(memKey)) {
      return _anilistFranchiseTvIds.get(memKey);
    }

    const ids = new Set();
    const queue = [root];
    while (queue.length && ids.size < maxNodes) {
      const id = queue.shift();
      if (!Number.isFinite(id) || ids.has(id)) continue;
      ids.add(id);

      const node = await fetchAnilistSeasonNode(id);
      if (!node?.relations?.edges) continue;
      ...
      for (const edge of node.relations.edges) {
        ...
        const fmt = anilistSeasonFormat(n.format);
        if (fmt !== "TV" && fmt !== "TV_SHORT" && fmt !== "ONA") continue;
        const nid = Number(n.id);
        if (Number.isFinite(nid) && !ids.has(nid)) queue.push(nid);
      }
    }

    const ordered = [...ids];
    _anilistFranchiseTvIds.set(memKey, ordered);
    return ordered;
  }

  function findFranchiseParentOnWatchlist(rootAnilistId, watchlistAnime = [], group = null) {
    const rootId = Number(rootAnilistId);
    if (!Number.isFinite(rootId)) return null;

    const direct = (watchlistAnime || []).find(
      (item) => resolveWatchlistItemAnilistIdSync(item) === rootId
    );
    if (direct) return direct;

    const franchiseIds = new Set(
      group?.allFranchiseTvIds?.length ? group.allFranchiseTvIds : group?.chainIds || [rootId]
    );
    const candidates = (watchlistAnime || []).filter((item) => {
      const itemId = resolveWatchlistItemAnilistIdSync(item);
      return itemId && franchiseIds.has(itemId);
    });
    if (!candidates.length) return null;

    candidates.sort((a, b) => {
      const aIsRoot = resolveWatchlistItemAnilistIdSync(a) === rootId ? 0 : 1;
      const bIsRoot = resolveWatchlistItemAnilistIdSync(b) === rootId ? 0 : 1;
  ...
  function computeAnimeGroupingDecision(group) {
    if (!group?.ok) {
      return {
        groupingDecision: "unresolved",
        shouldMerge: false,
        groupRole: null,
        standaloneReason: group?.reason || "unresolved",
      };
    }
    if (group.isRoot) {
      return {
        groupingDecision: "keep_as_parent",
        shouldMerge: false,
        groupRole: "root",
        standaloneReason: "franchise_root_card",
      };
    }
    return {
      groupingDecision: "merge_into_root",
      shouldMerge: true,
      groupRole: "child",
      standaloneReason: "",
    };
  ...
  async function probeWatchlistItemAnilistId(item) {
    const WM = window.WatchlistMetadata;
    const normalized = normalizeItemForAnilistResolve(item);
    const probe = {
      syncId: resolveWatchlistItemAnilistIdSync(item),
      providerCacheHit: false,
      providerCacheId: null,
      animeTitleIndexHit: false,
      animeTitleIndexId: null,
      malLinkId: null,
      liveSearchId: null,
    };
    if (!normalized) return probe;

    const title = String(normalized.title || "").trim();
    if (title && WM?.lookupCachedAnilistMatch) {
      const cached = WM.lookupCachedAnilistMatch(title, { searchPasses: [title] });
      const cacheId = Number(cached?.pick?.anilistId);
      if (Number.isFinite(cacheId) && cacheId > 0) {
        probe.providerCacheHit = true;
        probe.providerCacheId = cacheId;
      }
    }

    if (title && WM?.searchAnimeOfflineIndex) {
      try {
        const offline = await WM.searchAnimeOfflineIndex(title, {
          year: normalized.year ?? null,
        });
        const pickId = Number(offline?.pick?.anilistId);
        if (Number.isFinite(pickId) && pickId > 0) {
          probe.animeTitleIndexHit = true;
          probe.animeTitleIndexId = pickId;
        }
      } catch {
        /* probe only */
      }
    ...
    const malId = WM?.extractMalId?.(normalized.link || normalized.imdbLink);
    if (malId) probe.malLinkId = malId;

    return probe;
  }

  function anilistNodePoster(node) {
    const WM = window.WatchlistMetadata;
    const raw = node?.coverImage?.large || "";
    return WM?.upgradePosterForStorage?.(raw, { source: "anilist", anilistId: node?.id }) || raw;
  }

  function normalizeItemForAnilistResolve(input) {
    if (typeof input === "number" || typeof input === "string") {
      const id = Number(input);
      return Number.isFinite(id) && id > 0 ? { anilistId: id } : null;
    }
    if (typeof input !== "object") return null;
    const anilistId =
      input.anilistId ??
      input.details?.anilistId ??
      input.pick?.anilistId ??
      null;
    const providerId = input.providerId ?? input.provider_id ?? null;
    let providerAnilistId = NaN;
    if (providerId != null && providerId !== "") {
      const raw = String(providerId).trim();
      if (/^\d+$/.test(raw)) providerAnilistId = Number(raw);
      else {
        const fromProvider = window.WatchlistMetadata?.extractAnilistId?.(raw);
        providerAnilistId = Number(fromProvider);
      }
    }
    return {
      anilistId: anilistId ?? (Number.isFinite(providerAnilistId) ? providerAnilistId : null),
      link: input.link || input.details?.link || "",
      imdbLink: input.imdbLink || input.details?.imdbLink || "",
      title: input.title || input.details?.title || "",
      year: input.year ?? input.details?.year ?? null,
      contentType: input.contentType || input.details?.contentType || "",
    };
  }

  function resolveWatchlistItemAnilistIdSync(input) {
    const item = normalizeItemForAnilistResolve(input);
    if (!item) return null;
    const direct = Number(item.anilistId);
    if (Number.isFinite(direct) && direct > 0) return direct;
    const WM = window.WatchlistMetadata;
    const fromLink = Number(WM?.extractAnilistId?.(item.link));
    if (Number.isFinite(fromLink) && fromLink > 0) return fromLink;
    const fromImdbLink = Number(WM?.extractAnilistId?.(item.imdbLink));
    if (Number.isFinite(fromImdbLink) && fromImdbLink > 0) return fromImdbLink;
    return null;
  }
  ...
  function persistResolvedAnilistIdOnItem(item, anilistId, source = "") {
    ...
    if (WM?.cacheResolvedPreview) {
      WM.cacheResolvedPreview(
        ...
        { anilistId: id, source: "anilist", title: item.title || "" }
      );
    }
    if (changed && source) {
      item.anilistIdSource = source;
    }
    return changed;
  }

  async function resolveWatchlistItemAnilistId(input, options = {}) {
    const item = normalizeItemForAnilistResolve(input);
    if (!item) return { anilistId: null, source: "none" };

    const syncId = resolveWatchlistItemAnilistIdSync(item);
    if (syncId) {
      if (options.persist !== false && typeof input === "object" && input) {
        persistResolvedAnilistIdOnItem(input, syncId, "stored");
      }
      return { anilistId: syncId, source: "stored" };
    }
    ...
    if (title && WM?.lookupCachedAnilistMatch) {
      ...
      if (Number.isFinite(cacheId) && cacheId > 0) {
        if (options.persist !== false && typeof input === "object" && input) {
          persistResolvedAnilistIdOnItem(input, cacheId, "provider_cache");
          if (cached?.details?.poster) {
            WM.cacheResolvedPreview(
              { source: "anilist", anilistId: cacheId },
              { ...cached.details, anilistId: cacheId, source: "anilist" }
            );
          }
        }
        return { anilistId: cacheId, source: "provider_cache" };
      }
    }

    if (allowOffline && title && WM?.searchAnimeOfflineIndex) {
      try {
        const offline = await WM.searchAnimeOfflineIndex(title, { year: item.year ?? null });
        const pickId = Number(offline?.pick?.anilistId);
        if (Number.isFinite(pickId) && pickId > 0) {
          if (options.persist !== false && typeof input === "object" && input) {
            persistResolvedAnilistIdOnItem(input, pickId, "anime_title_index");
          }
          return { anilistId: pickId, source: "anime_title_index" };
        }
      } catch (error) {
        // Silent on expected offline/lookup misses — enable watchlist-debug-add for noise.
        if (typeof localStorage !== "undefined" && localStorage.getItem("watchlist-debug-add") === "1") {
          console.warn("[anime-group-debug] offline index lookup failed:", title, error);
        }
      }
    }

    const malId = WM?.extractMalId?.(item.link || item.imdbLink);
    if (malId && allowLive && WM?.fetchAnilistByMalId) {
      try {
        const details = await WM.fetchAnilistByMalId(malId);
        const malResolved = Number(details?.anilistId);
        if (Number.isFinite(malResolved) && malResolved > 0) {
          if (options.persist !== false && typeof input === "object" && input) {
            persistResolvedAnilistIdOnItem(input, malResolved, "mal_link");
          }
          return { anilistId: malResolved, source: "mal_link" };
        }
      } catch (error) {
        if (typeof localStorage !== "undefined" && localStorage.getItem("watchlist-debug-add") === "1") {
          console.warn("[anime-group-debug] MAL→AniList lookup failed:", title, error);
        }
      }
    }

    if (allowLive && title && WM?.fetchAnilistMatchByTitle) {
      try {
        const match = await WM.fetchAnilistMatchByTitle(title, item.year);
        const liveId = Number(match?.anilistId);
        if (Number.isFinite(liveId) && liveId > 0) {
          if (options.persist !== false && typeof input === "object" && input) {
            persistResolvedAnilistIdOnItem(input, liveId, "anilist_search");
          }
          return { anilistId: liveId, source: "anilist_search" };
        }
      } catch (error) {
        if (typeof localStorage !== "undefined" && localStorage.getItem("watchlist-debug-add") === "1") {
          console.warn("[anime-group-debug] AniList title search failed:", title, error);
        }
      }
    }

    return { anilistId: null, source: "unresolved" };
  }

  async function resolveAnimeSeriesGroup(input, options = {}) {
    let anilistId = resolveWatchlistItemAnilistIdSync(input);
    let resolveSource = "stored";

    if (!anilistId) {
      const resolved = await resolveWatchlistItemAnilistId(input, options);
      anilistId = resolved.anilistId;
      resolveSource = resolved.source;
    } else if (options.persist !== false && typeof input === "object" && input) {
      persistResolvedAnilistIdOnItem(input, anilistId, "stored");
    }
    ...
    if (franchise.incomplete || franchise.rateLimited || !franchise.rootAnilistId) {
      return {
        ok: false,
        reason: franchise.rateLimited
          ? "rate_limited"
          : franchise.incomplete
            ? "incomplete_franchise_root"
            : "no_franchise_root",
        resolveSource,
        anilistId: id,
        rootAnilistId: null,
        ...
        seasonNumber: null,
        relationType: null,
        rateLimited: Boolean(franchise.rateLimited),
      };
    }

    const rootId = Number(franchise.rootAnilistId);
    const groupingOnly = Boolean(options.groupingOnly);
    const includeChain = options.includeChain !== false;

    let chainIds = [];
    let members = [];
    let allFranchiseTvIds = [];

    if (groupingOnly) {
      if (includeChain) {
        chainIds = readCachedChainIdsFromSeriesMetadata(rootId) || [];
        if (!chainIds.length && id === rootId) chainIds = [rootId];
        members = chainIds.map((chainId, index) => ({
          anilistId: chainId,
      ...
      const media = await fetchAnilistSeasonNode(rootId);
      const chain = media ? await collectAnilistSeasonChain(media) : [];
      chainIds = chain.map((node) => Number(node.id)).filter(Number.isFinite);
      allFranchiseTvIds = await collectAnimeFranchiseTvIds(rootId);
      members = chain.map((node, index) => ({
        anilistId: Number(node.id),
        seasonNumber: index + 1,
        title:
          node.title?.english || node.title?.romaji || node.title?.native || "",
        episodes: parsePositiveCount(node.episodes),
        poster: anilistNodePoster(node),
        format: anilistSeasonFormat(node.format),
      }));
    }

    const childIndex = chainIds.indexOf(id);
    const isRoot = id === rootId;
    const isFranchiseChild = !isRoot;
    let seasonNumber = null;
    if (childIndex > 0) {
      seasonNumber = childIndex + 1;
    } else if (childIndex === 0) {
      seasonNumber = 1;
    } else if (isFranchiseChild) {
      seasonNumber = chainIds.length + 1;
    }

    const grouping = computeAnimeGroupingDecision({
      ok: true,
      isRoot,
      isFranchiseChild,
    });
    ...
    return {
      ...
      rootTitle: franchise.rootTitle || members[0]?.title || "",
      chainIds,
      allFranchiseTvIds,
      members,
      isRoot,
  ...
  async function findAnimeParentOnList(childAnilistId, watchlistAnime = []) {
    ...
    const parent = findFranchiseParentOnWatchlist(
      group.rootAnilistId,
      watchlistAnime,
      group
    );
    ...
    return {
      parent,
      rootAnilistId: group.rootAnilistId,
      childAnilistId: group.anilistId,
      seasonNumber: group.seasonNumber,
      relationType: group.relationType,
      relationPath: group.relationPath,
    };
  }
  ...
  window.WatchlistSeriesMetadata = {
    ...
    resolveLinkedImdbId,
    resolveAnimeFranchiseRoot,
    buildAnimeFranchiseRootMap,
    buildBatchFranchiseRootMapFromCache,
    reconcileAnimeFranchiseRootMap,
    readCachedChainIdsFromSeriesMetadata,
    readFranchiseRootCacheEntry,
    resolveWatchlistItemAnilistId,
    resolveWatchlistItemAnilistIdSync,
    resolveAnimeSeriesGroup,
    persistResolvedAnilistIdOnItem,
    ...
    isUsableAnimePosterUrl,
    pickAnimeMainPosterFallback,
    findAnimeParentOnList,
