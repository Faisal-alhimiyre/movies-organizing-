(function () {
  ...
  async function fetchAnilistByTitleMatch(title, year) {
    const score = await fetchAnilistScoreByTitle(title, year);
    if (!score) return null;
    return { source: "anilist", anilistRating: score };
  }
  ...
  async function fetchAnilistScoreByTitle(title, year) {
    ...
    if (match?.averageScore == null) return null;
    return String(match.averageScore);
  }
